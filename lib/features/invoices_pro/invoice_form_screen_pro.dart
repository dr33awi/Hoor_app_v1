...
class _InvoiceFormScreenProState extends ConsumerState<InvoiceFormScreenPro> {
  ...
  Widget build(BuildContext context) {
    // التحقق من وجود وردية مفتوحة (فقط لفواتير البيع)
    if (widget.isSales) {
      ...
      return shiftAsync.when(
        loading: () => Scaffold(
          ...
          appBar: ProAppBar.simple(title: 'فاتورة بيع جديدة'),
          body: ProLoadingState.withMessage(message: 'جاري التحميل...'),
        ),
        error: (e, s) => Scaffold(
          backgroundColor: AppColors.background,
          appBar: ProAppBar.simple(title: 'فاتورة بيع جديدة'),
          body: ProEmptyState.error(
            error: e.toString(),
            onRetry: () => ref.invalidate(openShiftStreamProvider),
          ),
        ),
        data: (shift) {
          if (shift == null) {
            return _buildNoShiftScreen();
          }
    ...
    // فواتير الشراء لا تتطلب وردية مفتوحة
    return _buildInvoiceForm();
  }

  /// شاشة عدم وجود وردية مفتوحة
  Widget _buildNoShiftScreen() {
    return Scaffold(
      backgroundColor: AppColors.background,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'يجب فتح وردية قبل إنشاء فاتورة بيع',
      ...
      appBar: ProAppBar.close(
        ...
        actions: [
          ProAppBarAction(
            ...
            onPressed: _isPreviewing || _items.isEmpty ? null : _previewInvoice,
            tooltip: 'معاينة',
          ),
        ],
      ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4.h),
                    // سعر قابل للنقر للتعديل
                    InkWell(
                      onTap: () => _showEditPriceDialog(item, index),
  ...
  void _showEditPriceDialog(Map<String, dynamic> item, int index) {
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            ...
            SizedBox(width: AppSpacing.sm),
            Text('تعديل السعر', style: AppTypography.titleMedium),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
                              ...
                              onTap: alreadyAdded || product.quantity == 0
                                  ...
                                  : () async {
                                      ...
                                      if (widget.isSales && price <= 0) {
                                        Navigator.pop(context);
                                        final enteredPrice =
                                            await _showPriceInputDialog(
                                                product.name);
                                        if (enteredPrice == null ||
                                            enteredPrice <= 0) {
                                          if (mounted) {
                                            ProSnackbar.warning(
                                                context, 'يجب إدخال سعر البيع');
                                          }
                                          return;
                                        }
                                        price = enteredPrice;
  ...
  /// نافذة إدخال سعر البيع للمنتجات التي ليس لها سعر
  Future<double?> _showPriceInputDialog(String productName) async {
    final priceController = TextEditingController();
    ...
    return showDialog<double>(
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            Icon(Icons.price_change_rounded, color: AppColors.warning),
            SizedBox(width: AppSpacing.sm),
            Expanded(
              child: Text(
                'أدخل سعر البيع',
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'هذا المنتج ليس له سعر بيع محدد',
                      style: AppTypography.bodySmall.copyWith(
                        color: AppColors.warning,
                      ),
                    ),
  ...
  Future<void> _previewInvoice() async {
    if (_items.isEmpty) {
      ProSnackbar.warning(context, 'أضف منتجات للفاتورة أولاً');
      return;
    }
    ...
    try {
      // إنشاء بيانات فاتورة مؤقتة للمعاينة
      final tempInvoice = _TempInvoiceForPreview(
        ...
        paidAmount: _paymentMethod == 'cash'
            ? _total
            : (_paymentMethod == 'credit' ? 0 : _paidAmount),
        paymentMethod: _paymentMethod,
        notes: _notesController.text.trim().isEmpty
            ? null
            : _notesController.text.trim(),
      ...
      final tempItems = _items
          .map((item) => _TempInvoiceItemForPreview(
                productName: item['name'],
      ...
      // الحصول على بيانات العميل/المورد إن وجد
      Customer? customer;
      Supplier? supplier;
      ...
      final pdfBytes = await InvoicePdfGenerator.generateInvoicePdfBytes(
        ...
        customer: customer,
        supplier: supplier,
        options: const InvoicePrintOptions(),
      );
      ...
      if (mounted) {
        ...
        final shouldPrint = await showProConfirmDialog(
          ...
          message: 'تم إنشاء PDF بنجاح. هل تريد طباعة الفاتورة؟',
...
/// كلاس مؤقت لمعاينة الفاتورة قبل الحفظ
class _TempInvoiceForPreview {
...
/// كلاس مؤقت لعناصر الفاتورة للمعاينة
class _TempInvoiceItemForPreview {
